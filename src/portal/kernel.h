...
namespace ssa::KernelHooks
{
        ...
        DWORD dwFlags, HANDLE hTemplate)
    {
        ...
        // ---- BLOCK real portal when emulated mode is active ----
        // portal.dll scans all HID devices in order and uses the first matching VID/PID
        // without this, a physically connected portal would be opened and used instead of our emulated backend, even though inject mode is active
        if (g_config.emulatedPortalStartup && LooksLikeRealPortal(lpFileName)) {
            Log("[Portal] Blocking real portal access (emulated mode active)");
        ...
        HANDLE hFile, LPOVERLAPPED lpOverlapped, LPDWORD lpBytesTransferred, BOOL bWait)
    {
        ...
        if (g_pending_real_buf && bytes > 0) {
            Portal::SniffIncoming(static_cast<const BYTE*>(g_pending_real_buf), static_cast<int>(bytes));
