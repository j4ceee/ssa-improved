...
namespace ssa::SetupApiHooks
{
    ...
    {
        if (!Portal::g_init_attempted && GuidIsHid(InterfaceClassGuid)) {
            Log("[Portal] First HID scan detected, lazy-initializing backends...");
            Portal::g_init_attempted = true;
            Portal::Init();
        }
